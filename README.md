...
The protected source and latest published release are **v0.34.38**. Release Please PR #1966 is
merged; release-assets workflow #32670939285 and the independent strict verifier completed all
...
The v0.34.38 package is a metadata-only follow-up to v0.34.37 and carries its verified runtime evidence and the approved-answer-term
...
The audited host installation now reports `cortana 0.34.38`; embedding and HTTP services are
...
contracts. The installed v0.34.38 binary passed the deterministic offline fixture evaluation and
packaged-core verification. A fresh v0.34.38 `cortana eval --model` run on 2026-08-23 passed in
25,682 ms with planner/synthesis, valid citations, cache reuse, and revision invalidation. On the
same day, one earlier direct run failed closed at
...
package for your operating system and CPU. The current published release is **v0.34.38**; its
...
The v0.34.38 Desktop support matrix is **macOS Apple Silicon (arm64), Linux x86_64, and Windows
x86_64**. v0.34.38 does not publish an Intel macOS Desktop bundle; Intel macOS is unsupported for
