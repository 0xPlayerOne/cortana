...
The protected source and latest published release are **v0.34.18**. The v0.34.18 release-assets
workflow completed the platform-package and strict 18-asset verification gate. The v0.34.18
...
The audited host installation now reports `cortana 0.34.18`, matching the latest verified package.
Embedding, HTTP, and backup services are healthy in query-only mode; recurring sync remains
uninstalled. The v0.34.18 readiness, native-memory, shared-agent, MCP, and control-plane evidence is
recorded in the linked operations and evaluation guides. The installed v0.34.18 binary also
...
package for your operating system and CPU. The current protected release is **v0.34.18**. Its
...
The v0.34.18 Desktop support matrix is **macOS Apple Silicon (arm64), Linux x86_64, and Windows
x86_64**. v0.34.18 does not publish an Intel macOS Desktop bundle; Intel macOS is unsupported for
