...
The protected source and latest published release are **v0.34.17**. Release-assets workflow
`32593385885` completed the platform-package and strict 18-asset verification gate. The v0.34.17
...
The audited host installation now reports `cortana 0.34.17`, matching the latest verified package.
Embedding, HTTP, and backup services are healthy in query-only mode; recurring sync remains
uninstalled. The v0.34.17 readiness, native-memory, shared-agent, MCP, and control-plane evidence is
recorded in the linked operations and evaluation guides. The installed v0.34.17 binary also
...
package for your operating system and CPU. The current protected release is **v0.34.17**. Its
...
The v0.34.17 Desktop support matrix is **macOS Apple Silicon (arm64), Linux x86_64, and Windows
x86_64**. v0.34.17 does not publish an Intel macOS Desktop bundle; Intel macOS is unsupported for
