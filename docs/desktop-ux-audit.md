...
- `v0.34.11` is the current protected source/release, published through the protected promotion
  and Release Please automation. Release-assets workflow `32546628726` completed all 18 assets,
...
- The current macOS Apple Silicon package verifier also passed the v0.34.11 Tauri updater
...
- The supported v0.34.11 Desktop matrix is macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
- The v0.34.11 tag is the packaged evidence boundary. The audited host now runs
  `/Users/amf/.local/bin/cortana` v0.34.11 with embedding and server services in query-only mode;
...
- The installed v0.34.11 host passed the bounded provider-backed `eval --model` evidence run on
  2026-08-22 with 13,211 ms answer latency: retrieval recall, MRR, case pass rate, citation validity,
...
The current v0.34.11 source includes the post-v0.31.12 safety lane, which acquires
...
The v0.34.11 source includes the operational recovery change for the local embedding supervisor:
...
and the validation remains below the production gate. The published v0.34.11 release contains the
...
2. Model-backed provider gate: the verified v0.34.11 package passed the credential-free packaged-core
   evaluator and query-only readiness in release verification. The installed v0.34.11 CLI passed
   the bounded provider-backed run on 2026-08-22 with 13,211 ms answer latency, planner/synthesis, valid
...
does not describe the current v0.34.11 source or the historical v0.32.6 core; use the
...
- The v0.30.10 release snapshot (tag commit `b46dda8`, workflow `31515684053`)
  ...
  the current `v0.34.11` source or packaged Desktop behavior. The verified
  `v0.32.12` asset workflow is historical; the active `v0.34.11` workflow is recorded in the release section above.
