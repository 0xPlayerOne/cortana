...
- `v0.34.3` is the current protected source/release, published through the protected promotion
  and Release Please automation. Release-assets workflow `32516622075` completed all 18 assets,
...
- The current macOS Apple Silicon package verifier also passed the v0.34.3 Tauri updater
...
- The supported v0.34.3 Desktop matrix is macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
- The v0.34.3 tag is the fully verified packaged evidence boundary. The audited host now runs
  `/Users/amf/.local/bin/cortana` v0.34.3 with embedding and server services in query-only mode;
  ...
  fetching stalled. The protected v0.34.3 release includes bounded parallel body fetching from PR #1594 and is now installed; no full-corpus reconciliation
...
- The installed v0.34.3 host passed the bounded provider-backed `eval --model` evidence run on
  2026-08-21 in 16,420 ms: retrieval recall, MRR, case pass rate, citation validity,
...
The current v0.34.3 source includes the post-v0.31.12 safety lane, which acquires
...
The v0.34.3 source adds one operational recovery change for the local embedding supervisor:
...
and the validation remains below the production gate. The v0.34.3 release containing the protected
...
2. Model-backed provider gate: the verified v0.34.3 package passed the credential-free packaged-core
   evaluator and query-only readiness in release verification. The installed v0.34.3 CLI passed
   the bounded provider-backed run on 2026-08-21 in 16,420 ms with planner/synthesis, valid
...
does not describe the current v0.34.3 source or the historical v0.32.6 core; use the
...
- The v0.30.10 release snapshot (tag commit `b46dda8`, workflow `31515684053`)
  ...
  the current `v0.34.3` source or packaged Desktop behavior. The verified
  `v0.32.12` asset workflow is historical; the active `v0.34.3` workflow is recorded in the release section above.
