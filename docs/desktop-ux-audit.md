...
- `v0.32.7` is the current protected source/release, published through the protected promotion
  and Release Please automation. Release-assets workflow `31896801633` completed all 18 assets
...
- The v0.32.7 archive is the current fully verified packaged evidence boundary. The audited host
  now runs the installed `/Users/amf/.local/bin/cortana` v0.32.7 with embedding and server services in
...
The current v0.32.7 source and package include the post-v0.31.12 safety lane, which acquires
...
2. Model-backed provider gate: the installed v0.32.6 CLI (the prior installed package) passed the
   ...
   citations, cache reuse, and revision invalidation. The current v0.32.7 package passed the
...
- The v0.30.10 release snapshot (tag commit `b46dda8`, workflow `31515684053`)
  ...
  the current `v0.32.7` source or packaged Desktop behavior. The verified
  `v0.32.7` asset workflow is recorded in the release section above.
