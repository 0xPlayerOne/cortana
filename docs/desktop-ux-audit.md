...
- `v0.34.5` is the current protected source/release, published through the protected promotion
  and Release Please automation. Release-assets workflow `32528987169` completed all 18 assets,
...
- The current macOS Apple Silicon package verifier also passed the v0.34.5 Tauri updater
...
- The supported v0.34.5 Desktop matrix is macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
- The v0.34.5 tag is the fully verified packaged evidence boundary. The audited host now runs
  `/Users/amf/.local/bin/cortana` v0.34.5 with embedding and server services in query-only mode;
  ...
  fetching stalled. The protected v0.34.5 release includes bounded parallel body fetching from PR #1594 and is installed; no full-corpus reconciliation
...
- The installed v0.34.5 host passed the bounded provider-backed `eval --model` evidence run on
  2026-08-21 with 17,091 ms answer latency: retrieval recall, MRR, case pass rate, citation validity,
...
The current v0.34.5 source includes the post-v0.31.12 safety lane, which acquires
...
The v0.34.5 source includes the operational recovery change for the local embedding supervisor:
...
and the validation remains below the production gate. The published v0.34.5 release contains the
...
2. Model-backed provider gate: the verified v0.34.5 package passed the credential-free packaged-core
   evaluator and query-only readiness in release verification. The installed v0.34.5 CLI passed
   the bounded provider-backed run on 2026-08-21 with 17,091 ms answer latency, planner/synthesis, valid
...
does not describe the current v0.34.5 source or the historical v0.32.6 core; use the
...
- The v0.30.10 release snapshot (tag commit `b46dda8`, workflow `31515684053`)
  ...
  the current `v0.34.5` source or packaged Desktop behavior. The verified
  `v0.32.12` asset workflow is historical; the active `v0.34.5` workflow is recorded in the release section above.
