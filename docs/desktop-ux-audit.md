...
- `v0.34.12` is the current protected source/release, published through the protected promotion
  and Release Please automation. Release-assets workflow `32552400854` completed all 18 assets,
...
- The current macOS Apple Silicon package verifier also passed the v0.34.12 Tauri updater
...
- The supported v0.34.12 Desktop matrix is macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
- The v0.34.12 tag is the packaged evidence boundary. The audited host now runs
...
The current v0.34.12 source includes the post-v0.31.12 safety lane, which acquires
...
The v0.34.12 source includes the operational recovery change for the local embedding supervisor:
...
and the validation remains below the production gate. The published v0.34.12 release contains the
...
2. Model-backed provider gate: the verified v0.34.12 package passed the credential-free packaged-core
...
does not describe the current v0.34.12 source or the historical v0.32.6 core; use the
...
- The v0.30.10 release snapshot (tag commit `b46dda8`, workflow `31515684053`)
  ...
  the current `v0.34.12` source or packaged Desktop behavior. The verified
  `v0.32.12` asset workflow is historical; the active `v0.34.12` workflow is recorded in the release section above.
