...
- `v0.32.8` is the current protected source/release, published through the protected promotion
  and Release Please automation. Release-assets workflow `31903165576` completed all 18 assets
  and the strict package gate. Asset verification does not launch the
  packaged GUI or prove OS-level signing/notarization.
- The supported v0.32.8 Desktop matrix is macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
The current v0.32.8 source and package include the post-v0.31.12 safety lane, which acquires
...
The v0.32.8 source adds one operational recovery change for the local embedding supervisor:
...
messages). Personal Drive is undergoing a 2,000-document/128 MiB/1,800-second validation after a
previous 900-second timeout; Work Drive's latest 900-second validation failed closed at its connector
deadline. Personal/Special Drive and Personal/Special Gmail remain below their configured budgets.
...
- The v0.30.10 release snapshot (tag commit `b46dda8`, workflow `31515684053`)
  ...
  the current `v0.32.8` source or packaged Desktop behavior. The verified
  `v0.32.8` asset workflow is recorded in the release section above.
