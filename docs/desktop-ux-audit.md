...
## Current release evidence (2026-08-24)

- `v0.34.40` is the current protected source/release, published through the protected promotion and
  Release Please automation. Release-assets workflow `32675543606` published all macOS/Linux/Windows
  core and Desktop lanes; the strict cross-platform verifier passed all 18 assets, signatures, checksums,
...
- The supported v0.34.40 Desktop matrix is macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
- The v0.34.40 tag is the source/release boundary. The audited host currently runs
  `/Users/amf/.local/bin/cortana` v0.34.40 with embedding and server services in query-only mode;
...
- The prior v0.34.35 provider-backed fixture gate passed in 9,824 ms with planner/synthesis,
  valid citations, cache reuse, and revision invalidation. Earlier v0.34.32 runs included one
  invalid-citation failure that correctly fell back to extractive mode. These are synthetic records
  only; they do not query the personal index, authorize synchronization, or prove packaged GUI
  behavior. The approved-corpus provider gate remains open.
...
- After the v0.34.22 installation, the current Apple Notes folder-scoped sources were revalidated
...
The current v0.34.40 source includes the post-v0.31.12 safety lane, which acquires
...
The current v0.34.40 source includes the operational recovery change for the local embedding supervisor:
...
2. Model-backed provider gate: the prior v0.34.35 host passed the bounded synthetic
...
does not describe the current v0.34.40 source or the historical v0.32.6 core; use the
...
- The v0.30.10 release snapshot (tag commit `b46dda8`, workflow `31515684053`)
  ...
  the current `v0.34.40` source or packaged Desktop behavior. The verified
  `v0.32.12` asset workflow is historical; the active `v0.34.40` workflow is recorded in the release section above.
