...
- The supported v0.34.16 Desktop matrix is macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
The current v0.34.16 source includes the post-v0.31.12 safety lane, which acquires
...
The v0.34.16 source includes the operational recovery change for the local embedding supervisor:
...
and the validation remains below the production gate. The published v0.34.16 release contains the
...
does not describe the current v0.34.16 source or the historical v0.32.6 core; use the
...
the current `v0.34.16` source or packaged Desktop behavior. The verified
  `v0.32.12` asset workflow is historical; the active `v0.34.16` workflow is recorded in the release section above.
