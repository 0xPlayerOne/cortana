...
- `v0.34.19` is the current protected source/release, published through the protected promotion
  and Release Please automation. The release-assets workflow completed the 18-asset
...
- The v0.34.19 macOS Apple Silicon package verifier passed the Tauri updater
...
- The supported v0.34.19 Desktop matrix is macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
- The v0.34.19 tag is the source/release boundary. The audited host currently runs
  `/Users/amf/.local/bin/cortana` v0.34.19 with embedding and server services in query-only mode;
...
- The installed v0.34.19 host `eval --model` run passed planner and synthesis execution, citations,
...
- The installed v0.34.19 binary passed the disposable native-memory, shared-agent HTTP, shared-agent
...
- After the v0.34.19 installation, the current Apple Notes folder-scoped sources were validated
...
The current v0.34.19 source includes the post-v0.31.12 safety lane, which acquires
...
The v0.34.19 source includes the operational recovery change for the local embedding supervisor:
...
and the validation remains below the production gate. The published v0.34.19 release contains the
...
2. Model-backed provider gate: the installed v0.34.19 host now passes the bounded synthetic
...
does not describe the current v0.34.19 source or the historical v0.32.6 core; use the
...
- The v0.30.10 release snapshot (tag commit `b46dda8`, workflow `31515684053`)
  ...
  the current `v0.34.19` source or packaged Desktop behavior. The verified
  `v0.32.12` asset workflow is historical; the active `v0.34.19` workflow is recorded in the release section above.
