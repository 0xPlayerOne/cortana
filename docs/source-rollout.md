...
cleaned up and no index or reconciliation writes occurred. The protected main tree now includes the
bounded four-worker body-fetch pool and regression coverage from PR #1594; install the next patch
release before repeating the production-budget validation. Work Drive completed its 2,000-document/
