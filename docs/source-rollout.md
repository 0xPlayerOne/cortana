...
| Google Drive/Gmail                                                         | Production-budget records for Work Drive (478 documents/4,527,663 bytes), Work Gmail (7,395/34,494,647), Personal Gmail (430/1,563,456), Special Drive (97/290,353), and Special Gmail (214/995,335). A v0.32.3 bounded Work Drive retry completed 100 unchanged documents with 0 deletions; Personal Drive failed closed at the 899-second connector timeout. | Keep recurring sync disabled; Special Drive remains the only completed production-budget cloud trial. Continue one-source-at-a-time `--no-reconcile` trials. |
...
embedding supervisor recovered; query-only readiness passed afterward. A subsequent v0.32.3 run with
