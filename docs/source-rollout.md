...
| Google Calendar (`work-calendar`, `personal-calendar`, `special-calendar`) | Complete validation records: 2,208, 1,839, and 0 events respectively. 100-event Work and Personal Calendar no-reconcile resumes completed with 0 deletions; Special remains validation-only after earlier budget-limited indexing attempts.                                            | Resume one calendar at a time with the bounded, resumable ingest path and review status before reconciliation.                                                   |
...
| Google Drive/Gmail                                                         | Production-budget validation records exist for Work Drive (478 documents/4,527,663 bytes), Work Gmail (7,395/34,494,647), Personal Gmail (430/1,563,456), Special Drive (97/290,353), and Special Gmail (214/995,335). Personal Drive failed closed at the 899-second connector timeout. | Keep recurring sync disabled; Special Drive has the only completed production-budget cloud trial. Retry other cloud sources one at a time with `--no-reconcile`. |
...
The 2026-08-15 Work Drive retry emitted all 478 connector records but failed closed when the local
embedding connection dropped during ingestion. It used `--no-reconcile`, made no deletions, and the
embedding supervisor recovered; query-only readiness passed afterward. Controlled ingestion may
retain a completed prefix, so this remains a reliability observation rather than a successful trial.
