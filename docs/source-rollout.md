...
with fewer records naturally returned fewer documents). These records support bounded,
non-reconciling work only and are below the configured production budgets; earlier larger
production-budget results in this plan are retained as historical evidence. Consequently the
