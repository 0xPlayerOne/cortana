...
The current protected source and published tag are `v0.34.35`, promoted through the protected
staging → main flow and Release Please automation. Release-assets workflow `32655800967` published
...
indexed data, recurring-sync policy, or native-memory behavior. The audited host now runs v0.34.35.
...
uninstalled. The installed v0.34.35 `eval --model` provider gate passed on 2026-08-23 in 9,824 ms
...
After installing v0.34.35, a fresh validation-only sweep on 2026-08-23 repeated the same
...
The audited host now runs `/Users/amf/.local/bin/cortana` v0.34.35. The embedding and HTTP services
...
The current v0.34.35 source retains the post-v0.31.6 Apple Notes executable hardening and
...
The current v0.34.35 source tree also serializes Desktop settings and service-schedule writes through a
shared per-config lock, held across validation, backups, atomic replacement, and audit writing.
This protects concurrent Desktop windows/processes in the v0.34.35 source; it does
not authorize source ingestion or recurring sync.

The current v0.34.35 source retains bounded embedding-supervisor recovery: steady-state health checks
...
release evidence; use **Current release boundary** above for the v0.34.35 source and package
state.
