...
The v0.34.27 release gate verifies the packaged core offline without credentials. The fresh
provider-backed `cortana eval --model` run against the installed v0.34.27 binary passed retrieval
recall, MRR, case pass rate, citation validity, planner and synthesis execution, cache reuse, and
revision invalidation in 12,082 ms under the 55,000 ms bound, with no provider fallback. This is
...
The audited host now runs `/Users/amf/.local/bin/cortana` v0.34.27. The embedding and HTTP services
...
The v0.34.27 source retains the post-v0.31.6 Apple Notes executable hardening and
...
The current v0.34.27 source tree also serializes Desktop settings and service-schedule writes through a
shared per-config lock, held across validation, backups, atomic replacement, and audit writing.
This protects concurrent Desktop windows/processes in the v0.34.27 source; it does
not authorize source ingestion or recurring sync.

The v0.34.27 source adds bounded embedding-supervisor recovery: steady-state health checks
...
release evidence; use **Current release boundary** above for the v0.34.27 source and package
state.
