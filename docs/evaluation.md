...
The current protected source and published tag are `v0.34.6`, promoted through the protected
flow and Release Please automation. Release-assets workflow `32531597016` completed the 18-asset
release gate, including checksums, updater signatures, manifest, and packaged-core verification.
The v0.34.6 source carries the native agentic-memory layer and the
rollback-safe Hermes migration publication
and the bounded
read-only live-index evaluator.
The published v0.34.6 package is the current downloadable boundary, and the audited host now runs
v0.34.6 after an upgrade that restarted only Cortana-owned services. Query-only readiness passed
...
spool was cleaned up and no index or reconciliation writes occurred. The published v0.34.6 release
...
The v0.34.6 release gate verified the packaged core offline without credentials. A fresh
provider-backed evaluation on 2026-08-21 using the installed v0.34.6 binary passed retrieval recall,
MRR, case pass rate,
...
The audited host now runs `/Users/amf/.local/bin/cortana` v0.34.6. The embedding, HTTP, and backup
...
After the v0.34.6 install, `cortana readiness --max-backup-age-hours 48` passed on the audited
...
The same installed v0.34.6 binary also passed the disposable native-memory lifecycle drill
...
The v0.34.6 source retains the post-v0.31.6 Apple Notes executable hardening and
...
The current v0.34.6 source tree also serializes Desktop settings and service-schedule writes through a
shared per-config lock, held across validation, backups, atomic replacement, and audit writing.
This protects concurrent Desktop windows/processes in the v0.34.6 source; it does
not authorize source ingestion or recurring sync.

The v0.34.6 source adds bounded embedding-supervisor recovery: steady-state health checks
