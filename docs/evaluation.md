...
The current protected source and published tag are `v0.34.20`, promoted through the protected
staging → main flow and Release Please automation. Release-assets workflow `32603677864` completed
...
v0.34.15; it does not change credentials, source authorization, indexed data, recurring-sync policy,
or native-memory behavior. The audited host now runs v0.34.20 after an
upgrade that restarted only Cortana-owned services. Query-only readiness,
...
The current v0.34.20 Personal Drive follow-up confirms the distinction: validation passed at the
...
The v0.34.20 release gate verifies the packaged core offline without credentials. A fresh
provider-backed `cortana eval --model` run against the installed v0.34.20 binary passed retrieval
...
The audited host now runs `/Users/amf/.local/bin/cortana` v0.34.20. The embedding, HTTP, and backup
...
On 2026-08-22, the installed v0.34.20 binary passed the disposable native-memory lifecycle drill
...
On 2026-08-22, the installed v0.34.20 binary revalidated all three configured Apple Notes folder
...
The v0.34.20 source retains the post-v0.31.6 Apple Notes executable hardening and
...
The current v0.34.20 source tree also serializes Desktop settings and service-schedule writes through a
shared per-config lock, held across validation, backups, atomic replacement, and audit writing.
This protects concurrent Desktop windows/processes in the v0.34.20 source; it does
not authorize source ingestion or recurring sync.

The v0.34.20 source adds bounded embedding-supervisor recovery: steady-state health checks
...
release evidence; use **Current release boundary** above for the v0.34.20 source and verified package
state.
