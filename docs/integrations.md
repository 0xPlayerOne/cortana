...
| `GET /readyz`       | Public readiness: database stats and an embedding provider probe; `503` when unavailable                                                                                                                                                                                                            |
| `GET /v1/status`    | Index and ingestion status counters, filtered to the principal's ACL when scoped; requires the `status` scope                                                                                                                                                                                       |
| MCP `brain_status`  | Configured source inventory — names, kinds, projects, enabled state, ACL labels — without exposing credentials; filtered by the principal's ACL                                                                                                                                                     |
