...
| `Chunk`                | `id`, `document_id`, `ordinal`, `content`, `content_hash`, `chunk_key`, `strategy`, `parent_key`, `previous_key`, `next_key`, `start_byte`, `end_byte`, `policy_version` | Derived from a canonical document and rebuildable; lineage fields are optional for legacy rows                  |
...
| `MemoryRecord`         | `id`, compatibility `kind`, `content_type`, `retention_tier`, `scope`, `project`, content/provenance, validity, status, ACL, revision fields                             | Explicit operational conclusion; see [memory contract](memory.md)                                               |
