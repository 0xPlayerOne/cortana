...
`v0.34.6` is the current protected source and published release. Release-assets workflow
`32531597016` completed the archive, checksum, updater-signature, manifest, and credential-free
packaged-core gates; all 18 published assets are verified. v0.34.5 and earlier remain
historical evidence.

### Supported Desktop platforms

The v0.34.6 Desktop support policy is **macOS Apple Silicon (arm64), Linux x86_64, and Windows
...
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 \
  scripts/verify-desktop-release.sh v0.34.6
```
...
The v0.34.6 source is the release boundary for native agentic memory and the post-v0.31.12
...
reconciliation writes occurred. The protected v0.34.6 release includes bounded parallel body fetching from
...
## v0.34.6 release intent (published and verified)
