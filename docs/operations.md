...
On the current v0.34.38 source tree, mutating CLI startup acquires the same global `sync.lock`
before opening the store. This covers schema/backfill/fingerprint work as well as the later import
or sync operation. This is included in the published v0.34.38 source release.
...
included in the published v0.34.38 source release; keep the same lock requirement when running a newer
...
The current published release is `v0.34.38`; release-assets workflow `32670939285` completed all
platform lanes and the independent strict verifier passed all 18 assets, checksums, updater
signatures, manifest checks, and packaged-core checks. The audited host now runs v0.34.38, so
...
```bash
  GH_REPO=0xPlayerOne/cortana bun run desktop:verify:mac v0.34.38
```
...
`passed: true`. The v0.34.38 core archive and the full archive/signature/checksum/updater-manifest
release gate both passed in release-assets workflow `32670939285`. This remains separate from GUI,
...
### Current local source rollout snapshot (2026-08-23; published and installed v0.34.38)
...
Both cancelled attempts made zero index or reconciliation writes. The published v0.34.38 release
...
The installed v0.34.38 provider-backed `cortana eval --model` gate passed on 2026-08-23 in
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.34.38
```
