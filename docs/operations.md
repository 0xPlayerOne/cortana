...
On the current v0.34.34 source tree, mutating CLI startup acquires the same global `sync.lock`
before opening the store. This covers schema/backfill/fingerprint work as well as the later import
or sync operation. This is included in the published v0.34.34 source release.
...
included in the published v0.34.34 source release; keep the same lock requirement when running a newer
...
The current published release is `v0.34.34`; release-assets workflow `32654111401` and the strict
18-asset verifier passed. The audited host now runs v0.34.34, so host-install and personal-source
...
```bash
  GH_REPO=0xPlayerOne/cortana bun run desktop:verify:mac v0.34.34
```
...
`passed: true`. The v0.34.34 verifier recorded this packaged-core gate in addition to the
...
### Current local source rollout snapshot (2026-08-23; published and installed v0.34.34)
...
Both cancelled attempts made zero index or reconciliation writes. The published v0.34.34 release
...
The installed v0.34.34 provider-backed `cortana eval --model` gate is intermittent: one direct run
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.34.34
```
