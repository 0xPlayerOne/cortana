...
On the current v0.34.3 source tree, mutating CLI startup acquires the same global `sync.lock`
before opening the store. This covers schema/backfill/fingerprint work as well as the later import
or sync operation. This is included in the published v0.34.3 source release.
...
included in the published v0.34.3 source release; keep the same lock requirement when running a newer
...
```bash
  GH_REPO=0xPlayerOne/cortana bun run desktop:verify:mac v0.34.3
```
...
`passed: true`. The v0.34.3 verifier recorded this packaged-core gate in addition to the
archive/signature/checksum/updater-manifest checks in workflow `32516622075`.
...
### Current local source rollout snapshot (2026-08-21; v0.34.3)
...
cancelled attempts made zero index or reconciliation writes. The protected v0.34.3 release includes bounded
...
The installed v0.34.3 CLI passed a fresh bounded `eval --model` run on 2026-08-21 in 16,420 ms,
...
The v0.34.3 source uses the local embedding `/health` endpoint for steady-state
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.34.3
```
