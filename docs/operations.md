...
v0.29.65; no functional source files changed. The installed v0.29.68 CLI is headless and passes
...
packaged Desktop app was not launched or replaced. Release-assets workflow `31457202736` published
all 18 v0.29.68 assets, and the fail-closed verifier passed their checksums, sidecars, macOS
resources, six Tauri updater signatures, and manifest.
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.29.68
```

The v0.29.68 core archive can also be verified independently:

```bash
scripts/verify-release.sh cortana-v0.29.68-aarch64-apple-darwin.tar.gz
```
