...
On the current v0.34.7 source tree, mutating CLI startup acquires the same global `sync.lock`
before opening the store. This covers schema/backfill/fingerprint work as well as the later import
or sync operation. This is included in the published v0.34.7 source release.
...
included in the published v0.34.7 source release; keep the same lock requirement when running a newer
...
`passed: true`. The v0.34.7 verifier recorded this packaged-core gate in addition to the
archive/signature/checksum/updater-manifest checks in workflow `32533269847`.
...
### Current local source rollout snapshot (2026-08-21; v0.34.7)
...
cancelled attempts made zero index or reconciliation writes. The published v0.34.7 release includes bounded
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.34.7
```
