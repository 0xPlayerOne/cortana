...
release-assets workflow `31474156961` completed all five platform jobs. The v0.30.2 and v0.30.7
release-assets workflows are historical evidence. The current v0.30.8 release-assets workflow
`31511779579` completed all five platform jobs; the strict
`GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.30.8`
check passed all 18 assets, signatures, checksums, and updater-manifest checks. The installed core
now reports v0.30.8; the packaged Desktop GUI has not been launched or replaced on this host. The
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.30.7
```
