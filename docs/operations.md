...
On the current v0.34.36 source tree, mutating CLI startup acquires the same global `sync.lock`
before opening the store. This covers schema/backfill/fingerprint work as well as the later import
or sync operation. This is included in the published v0.34.36 source release.
...
included in the published v0.34.36 source release; keep the same lock requirement when running a newer
...
```bash
  GH_REPO=0xPlayerOne/cortana bun run desktop:verify:mac v0.34.36
```
...
### Current local source rollout snapshot (2026-08-23; published and installed v0.34.36)
...
Both cancelled attempts made zero index or reconciliation writes. The published v0.34.36 release
...
The installed v0.34.36 provider-backed `cortana eval --model` gate is intermittent: one direct run
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.34.36
```
