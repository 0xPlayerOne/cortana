...
On the current v0.32.2 source tree, mutating CLI startup acquires the same global `sync.lock`
before opening the store. This covers schema/backfill/fingerprint work as well as the later import
or sync operation. This is included in the v0.32.2 package.
...
included in the v0.32.2 package; keep the same lock requirement when running a newer
...
```bash
GH_REPO=0xPlayerOne/cortana bun run desktop:verify:mac v0.32.2
```
...
`passed: true`. The published v0.32.2 verifier must pass this packaged-core gate in addition to the
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.32.2
```
