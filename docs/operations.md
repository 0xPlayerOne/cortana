...
On the current v0.34.1 source tree, mutating CLI startup acquires the same global `sync.lock`
before opening the store. This covers schema/backfill/fingerprint work as well as the later import
or sync operation. This is included in the published v0.34.1 source release.
...
included in the published v0.34.1 source release; keep the same lock requirement when running a newer
...
```bash
  GH_REPO=0xPlayerOne/cortana bun run desktop:verify:mac v0.34.1
```
...
`passed: true`. The v0.34.1 verifier recorded this packaged-core gate in addition to the
archive/signature/checksum/updater-manifest checks in workflow `32343629811`.
...
cancelled attempts made zero index or reconciliation writes. The protected main tree now includes bounded
four-worker fetching from PR #1594; install the next patch release before retrying, and the `readiness --allow-sync-service` gate must remain closed until a fresh complete record
...
The v0.34.1 source uses the local embedding `/health` endpoint for steady-state
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.34.1
```
