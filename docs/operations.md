...
On the current v0.34.20 source tree, mutating CLI startup acquires the same global `sync.lock`
before opening the store. This covers schema/backfill/fingerprint work as well as the later import
or sync operation. This is included in the published v0.34.20 source release.
...
included in the published v0.34.20 source release; keep the same lock requirement when running a newer
...
The installed v0.34.20 binary passed this drill on 2026-08-22. It created and searched the
...
`passed: true`. The v0.34.20 verifier records this packaged-core gate in addition to the
...
### Current local source rollout snapshot (2026-08-22; published and installed v0.34.20)
...
Both cancelled attempts made zero index or reconciliation writes. The published v0.34.20 release
...
A fresh provider-backed `cortana eval --model` run on the installed v0.34.20 binary passed planner
...
```bash
GH_REPO=0xPlayerOne/cortana CORTANA_REQUIRE_MINISIGN=1 scripts/verify-desktop-release.sh v0.34.20
```
